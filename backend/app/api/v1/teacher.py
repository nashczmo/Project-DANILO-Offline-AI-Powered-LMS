...
def teacher_gradebook(course_id: str, current_user: User=Depends(get_current_user), db: Session=Depends(get_db)) -> dict:
    ...
    return {'course': serialize_course(course), 'students': students, 'entries': [{'id': grade.id, 'studentId': student.id, 'studentName': student.full_name, 'term': grade.term, 'gradingPeriod': grade.grading_period, 'component': grade.component, 'score': grade.score, 'maxScore': grade.max_score, 'weight': grade.weight, 'remarks': grade.remarks or ''} for grade, student in grades], 'grades': summaries}
...
def teacher_create_grade(course_id: str, payload: dict=Body(default={}), current_user: User=Depends(get_current_user), db: Session=Depends(get_db)) -> dict:
    ...
    grade = GradeEntry(student_id=student_id, course_id=course_id, term=validate_term(payload.get('term') or 'Term 1'), grading_period=validate_grading_period(payload.get('gradingPeriod') or payload.get('grading_period') or 'Midterm'), component=clean_text(payload.get('component'), max_length=80), score=parse_float(payload.get('score'), 'Score', minimum=0), max_score=parse_float(payload.get('maxScore') or payload.get('max_score') or 100, 'Max score', minimum=1), weight=parse_float(payload.get('weight') or 1, 'Weight', minimum=0), remarks=clean_text(payload.get('remarks'), required=False, max_length=1000), recorded_by=current_user.id)
    db.add(grade)
    db.commit()
...
def teacher_update_grade(grade_id: str, payload: dict=Body(default={}), current_user: User=Depends(get_current_user), db: Session=Depends(get_db)) -> dict:
    ...
    for attr, key in [('term', 'term'), ('grading_period', 'gradingPeriod'), ('component', 'component'), ('remarks', 'remarks')]:
        val = payload.get(key) if key in payload else payload.get('grading_period') if attr == 'grading_period' else None
        if val is not None or key in payload or (attr == 'grading_period' and 'grading_period' in payload):
            if attr == 'term':
                grade.term = validate_term(val)
            elif attr == 'grading_period':
                grade.grading_period = validate_grading_period(val)
            else:
                setattr(grade, attr, clean_text(val, required=attr != 'remarks', max_length=1000 if attr == 'remarks' else 80))
