...
async def lifespan(_: FastAPI):
    ...
    try:
        ...
        logger.info('Backend startup: preparing background RAG indexing')
        def _background_rag():
            db_bg = SessionLocal()
            try:
                rebuild_rag_index(db_bg)
            finally:
                db_bg.close()
                
        asyncio.create_task(asyncio.to_thread(_background_rag))
    finally:
        db.close()
...
def validate_grading_period(value: str | None) -> str:
    period = clean_text(value or 'Midterm', max_length=10)
    if period not in {'Midterm', 'Endterm'}:
        raise HTTPException(status_code=400, detail='Grading period must be Midterm or Endterm')
    return period
...
def _compute_period_grade(components: list[dict]) -> float | None:
    """Compute weighted average grade for a list of grade components."""
    weighted_score = 0.0
    weight_total = 0.0
    for comp in components:
        weighted_score += comp['percentage'] * comp['weight']
        weight_total += comp['weight']
    return round(weighted_score / weight_total, 2) if weight_total else None

def summarize_grade_entries(course: Course, rows: list[GradeEntry], student_name: str | None=None) -> list[dict]:
    # Group by term, then by grading period within each term
    term_buckets: dict[str, dict] = {}
    for grade in rows:
        grading_period = getattr(grade, 'grading_period', 'Midterm') or 'Midterm'
        bucket = term_buckets.setdefault(grade.term, {
            'courseId': course.id, 'courseCode': course.code, 'courseTitle': course.title,
            'subject': course.subject, 'term': grade.term, 'schoolYear': course.school_year,
            'teacher': course.teacher.full_name if course.teacher else '', 'studentName': student_name,
            'components': [], 'midtermComponents': [], 'endtermComponents': []
        })
        normalized = grade.score / grade.max_score * 100.0 if grade.max_score else 0.0
        comp_entry = {
            'id': grade.id, 'component': grade.component, 'gradingPeriod': grading_period,
            'score': grade.score, 'maxScore': grade.max_score, 'weight': grade.weight,
            'remarks': grade.remarks or '', 'percentage': round(normalized, 2)
        }
        bucket['components'].append(comp_entry)
        if grading_period == 'Endterm':
            bucket['endtermComponents'].append(comp_entry)
        else:
            bucket['midtermComponents'].append(comp_entry)
    summary = []
    for bucket in term_buckets.values():
        midterm_grade = _compute_period_grade(bucket['midtermComponents'])
        endterm_grade = _compute_period_grade(bucket['endtermComponents'])
        # Term Grade = (Midterm + Endterm) / 2
        if midterm_grade is not None and endterm_grade is not None:
            final_grade = round((midterm_grade + endterm_grade) / 2.0, 2)
        elif midterm_grade is not None:
            final_grade = midterm_grade
        elif endterm_grade is not None:
            final_grade = endterm_grade
        else:
            final_grade = None
        bucket['midtermGrade'] = midterm_grade
        bucket['endtermGrade'] = endterm_grade
        bucket['finalGrade'] = final_grade
...
def build_grade_summary(db: Session, student_id: str) -> list[dict]:
    ...
    for grade, course in rows:
        ...
        normalized = grade.score / grade.max_score * 100.0 if grade.max_score else 0.0
        comp_entry = {
            'component': grade.component, 'gradingPeriod': grading_period,
            'score': grade.score, 'maxScore': grade.max_score, 'weight': grade.weight,
            'remarks': grade.remarks or '', 'percentage': round(normalized, 2)
        }
        bucket['components'].append(comp_entry)
