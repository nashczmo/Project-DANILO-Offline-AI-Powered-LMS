...
install_docker() {
  ...
  if command_missing docker || ! docker compose version >/dev/null 2>&1; then
    ...
    if ! curl -fsS -I "https://download.docker.com/linux/ubuntu/dists/${codename}/stable/" >/dev/null 2>&1; then
      note "Docker repository does not yet support Ubuntu codename '${codename}'; falling back to 'jammy' (22.04 LTS)"
      codename="jammy"
    fi
...
EOF
    ...
    # Ensure apt updates after repository change, clearing the once flag
    export APT_UPDATED_ONCE=0
...
install_node() {
  ...
  if [[ "${current_major}" -lt 20 ]]; then
    ...
    # Note: NodeSource no longer uses dist codenames directly in its standard setup for modern versions,
    # it uses 'nodistro'. So it is widely compatible across versions.
...
EOF
    ...
    # Ensure apt updates after repository change
    export APT_UPDATED_ONCE=0
    if declare -f apt_update_once >/dev/null; then
      apt_update_once
    else
...
prefetch_container_assets() {
  ...
  if [[ "${DANILO_AI_RUNTIME:-ollama}" == "ollama" ]]; then
    if [[ "${DANILO_AI_ENABLE:-0}" -eq 1 ]]; then
      ...
      note "Skipping heavy Ollama model preload (Fast Install mode). Set DANILO_AI_ENABLE=1 or use --full-install to override."
    fi
  else
