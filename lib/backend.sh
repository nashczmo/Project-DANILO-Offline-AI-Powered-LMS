...
def _detected_hardware() -> dict:
    ...
    return {
        "profile": profile,
        "cpuModel": os.getenv("DANILO_AI_CPU_MODEL", "unknown"),
        "ramMb": ram_mb,
        "ramAvailableMb": ram_available_mb,
        "cpuCount": cpu_count,
        "gpuName": os.getenv("DANILO_AI_GPU_NAME", "none"),
        "gpuVramMb": gpu_vram_mb,
        "integratedGpu": bool(integrated_gpu),
        "dedicatedGpu": bool(dedicated_gpu),
        "cuda": bool(cuda),
        "rocm": bool(rocm),
        "avx2": bool(avx2),
        "avx512": bool(avx512),
...
def _profile_defaults() -> dict:
    ...
    defaults = {
        "model": os.getenv("DANILO_AI_MODEL_LOW", "qwen2.5:1.5b"),
        "fallback_model": os.getenv("DANILO_AI_MODEL_BALANCED", "qwen2.5:3b"),
        "optional_model": "",
    ...
    if profile == "balanced":
        defaults.update({"model": os.getenv("DANILO_AI_MODEL_BALANCED", "qwen2.5:3b"), "fallback_model": os.getenv("DANILO_AI_MODEL_LOW", "qwen2.5:1.5b"), "optional_model": os.getenv("DANILO_AI_MODEL_GPU", "qwen2.5:7b"), "model_class": "mid-cpu", "quantization": "q4_K_M", "timeout": 150.0, "ctx": 1536, "context_chars": 2400, "threads": max(1, min(cpu_count, 6)), "batch": 192, "scheduler": "fair-queue", "cache_size": 200, "cooldown": 4.0})
    elif profile == "high-memory":
        defaults.update({"model": os.getenv("DANILO_AI_MODEL_BALANCED", "qwen2.5:3b"), "fallback_model": os.getenv("DANILO_AI_MODEL_LOW", "qwen2.5:1.5b"), "optional_model": os.getenv("DANILO_AI_MODEL_GPU", "qwen2.5:7b"), "model_class": "mid-cpu", "quantization": "q4_K_M", "concurrency": 2, "timeout": 150.0, "ctx": 2048, "context_chars": 3000, "threads": max(1, min(cpu_count, 8)), "batch": 256, "scheduler": "cpu-throughput", "cache_size": 300, "cooldown": 3.0})
    elif profile == "gpu-accelerated":
        defaults.update({"model": os.getenv("DANILO_AI_MODEL_GPU", "qwen2.5:7b"), "fallback_model": os.getenv("DANILO_AI_MODEL_BALANCED", "qwen2.5:3b"), "optional_model": os.getenv("DANILO_AI_MODEL_LOW", "qwen2.5:1.5b"), "model_class": "gpu", "quantization": "q4_K_M", "concurrency": 2, "timeout": 120.0, "ctx": 2048, "context_chars": 3200, "threads": max(1, min(cpu_count, 8)), "gpu_layers": 999, "batch": 384, "scheduler": "gpu-balanced", "cache_size": 400, "cooldown": 2.0})
    elif profile == "large-gpu":
        defaults.update({"model": os.getenv("DANILO_AI_MODEL_HIGH", "qwen2.5:14b"), "fallback_model": os.getenv("DANILO_AI_MODEL_GPU", "qwen2.5:7b"), "optional_model": os.getenv("DANILO_AI_MODEL_BALANCED", "qwen2.5:3b"), "model_class": "large", "quantization": "q5_K_M", "concurrency": 3, "timeout": 120.0, "ctx": 4096, "context_chars": 5600, "threads": max(1, min(cpu_count, 8)), "gpu_layers": 999, "batch": 512, "kv_cache": "f16", "scheduler": "gpu-throughput", "cache_size": 600, "cooldown": 1.5})
...
# Runtime profile is detected by the installer and re-evaluated here when env
# values are set to "auto", so the same backend can run on low-end CPUs,
# mid-range boxes, GPU hosts, and upgraded future hardware.
# ---------------------------------------------------------------------------
...
RESPONSE_MODE_OPTIONS = {
    ...
    "simplify":     {"num_predict": 250, "instruction": "Answer using very simple language, short sentences, and elementary concepts. Do not use jargon."},
    "step_by_step": {"num_predict": 500, "instruction": "Explain the concept clearly in sequential steps. Number each step."},
    "filipino":     {"num_predict": 400, "instruction": "Explain the concept clearly using conversational Filipino (Taglish is okay). Ensure the tone is encouraging."},
    "reviewer":     {"num_predict": 500, "instruction": "Generate a concise review summary. Use bullet points for key facts, formulas, or takeaways."},
    "quiz_me":      {"num_predict": 300, "instruction": "Generate a quick 3-question multiple-choice quiz about this topic. Do not give the answers immediately."},
...
def build_rolling_memory(db: Session, user_id: int, course_id: int | None, limit: int) -> list[dict]:
    ...
    memory = []
    # Keep rolling memory proportional to the selected context window so stronger
    # hardware gets better continuity while constrained hosts stay responsive.
    char_budget = _env_int("DANILO_MEMORY_CHAR_BUDGET", max(600, min(1800, OLLAMA_CONTEXT_CHARS // 2)), minimum=0, maximum=6000)
    current_chars = 0
...
_RAG_STOPWORDS = {
    ...
    "how", "why", "are", "was", "were", "ang", "ng", "mga", "sa", "na", "at", "ay", "para",
}


def _rag_tokens(value: str) -> list[str]:
    return [t for t in re.findall(r"[a-zA-Z0-9_]{3,}", value.lower()) if t not in _RAG_STOPWORDS]
...
def _rag_chunks(module: Module, chunk_size: int = 900, overlap: int = 120) -> list[str]:
    source = "\n".join(
        ...
        for part in [
            module.title,
            module.learning_competency or "",
            module.summary,
            module.essential_question,
            module.content or "",
        ]
        if part
    )
...
def index_module_for_rag(module: Module) -> None:
    ...
    with sqlite3.connect(DANILO_AI_INDEX_PATH) as conn:
        ...
        for idx, chunk in enumerate(chunks):
            conn.execute(
                "INSERT INTO lesson_chunks(module_id, course_id, chunk_index, title, content, vector_json, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (module.id, module.course_id, idx, module.title, chunk, json.dumps(_rag_vector(chunk)), now),
            )
...
def ollama_chat_payload(prompt: str, mode: str, *, stream: bool, memory: list[dict] | None = None, model: str | None = None, gpu_layers: int | None = None) -> dict:
    ...
    return {
        ...
        "options": {
            ...
            "num_gpu": OLLAMA_NUM_GPU if gpu_layers is None else gpu_layers,
        },
    }
...
async def _post_nonstream_inference(client: httpx.AsyncClient, prompt: str, mode: str, memory: list[dict] | None, model_name: str, gpu_layers: int) -> tuple[str, dict]:
    payload = ollama_chat_payload(prompt, mode, stream=False, memory=memory, model=model_name, gpu_layers=gpu_layers)
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            for model_name, gpu_layers, route in _inference_attempts():
                try:
                    result, raw_metrics = await _post_nonstream_inference(client, prompt, mode, memory, model_name, gpu_layers)
                    break
                except Exception as exc:
                    last_error = exc
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            for model_name, gpu_layers, route in _inference_attempts():
                try:
                    full_parts: list[str] = []
                    pending = ""
                    payload = ollama_chat_payload(prompt, mode, stream=True, memory=memory, model=model_name, gpu_layers=gpu_layers)
...
def ai_status() -> dict:
    ...
    return {
        ...
        "modelClass": _AI_MODEL_CLASS,
        "quantization": _AI_QUANTIZATION,
        "scheduler": _AI_SCHEDULER,
        "ramAvailableMb": ram_available_mb,
        "cpuPercent": cpu_percent,
        "queueSlots": _AI_MAX_CONCURRENT,
        "queueDepth": _ai_queue_depth(),
        "queueTimeoutSeconds": _AI_QUEUE_TIMEOUT_SECONDS,
        "hardwareProfile": _HARDWARE["profile"],
        "cpuThreads": _HARDWARE["cpuCount"],
        "gpuVramMb": _HARDWARE["gpuVramMb"],
        "contextTokens": OLLAMA_NUM_CTX,
        "contextChars": OLLAMA_CONTEXT_CHARS,
        "inferenceThreads": OLLAMA_NUM_THREADS,
        "gpuLayers": OLLAMA_NUM_GPU,
        "batchSize": OLLAMA_NUM_BATCH,
        "kvCache": OLLAMA_KV_CACHE_TYPE,
        "cacheSize": len(_ai_response_cache),
        "requests": _AI_TOTAL_REQUESTS,
        "timeouts": _AI_TIMEOUTS,
        "errors": _AI_ERRORS,
        "lastModel": _AI_LAST_MODEL,
...
def admin_system_status(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "aiStatus": {
            "ready": ai_ready,
            "model": OLLAMA_MODEL,
            "message": "Ollama model is available" if ai_ready else f"Ollama is offline or the configured model is missing: {OLLAMA_MODEL}",
        },
        "aiRuntime": DANILO_AI_RUNTIME,
        ...
        "aiOptionalModel": DANILO_AI_OPTIONAL_MODEL,
        ...
        "aiQueueDepth": _ai_queue_depth(),
        "aiQueueTimeoutSeconds": _AI_QUEUE_TIMEOUT_SECONDS,
        "aiHardwareProfile": _HARDWARE["profile"],
        "aiModelClass": _AI_MODEL_CLASS,
        "aiQuantization": _AI_QUANTIZATION,
        "aiScheduler": _AI_SCHEDULER,
        "aiContextTokens": OLLAMA_NUM_CTX,
        "aiInferenceThreads": OLLAMA_NUM_THREADS,
        "aiGpuLayers": OLLAMA_NUM_GPU,
        "aiBatchSize": OLLAMA_NUM_BATCH,
        "aiKvCache": OLLAMA_KV_CACHE_TYPE,
        "aiCacheSize": len(_ai_response_cache),
        "aiRequests": _AI_TOTAL_REQUESTS,
        "aiTimeouts": _AI_TIMEOUTS,
        "aiErrors": _AI_ERRORS,
        "aiLastLatencyMs": _AI_LAST_LATENCY_MS,
        "ragIndexPath": DANILO_AI_INDEX_PATH,
        "ragIndexed": os.path.exists(DANILO_AI_INDEX_PATH),
        "hardware": {
            "cpuPercent": cpu_pct,
            ...
            "profile": _HARDWARE["profile"],
            "cpuModel": _HARDWARE["cpuModel"],
            "cpuThreads": _HARDWARE["cpuCount"],
            "gpuName": _HARDWARE["gpuName"],
            "gpuVramMb": _HARDWARE["gpuVramMb"],
            "integratedGpu": _HARDWARE["integratedGpu"],
            "dedicatedGpu": _HARDWARE["dedicatedGpu"],
            "cuda": _HARDWARE["cuda"],
            "rocm": _HARDWARE["rocm"],
            "avx2": _HARDWARE["avx2"],
            "avx512": _HARDWARE["avx512"],
            "vulkan": _HARDWARE["vulkan"],
            "opencl": _HARDWARE["opencl"],
            "storageAvailableMb": _HARDWARE["storageAvailableMb"],
        },
        "totals": {
...
def admin_ai_models(current_user: User = Depends(get_current_user)) -> dict:
    ...
    try:
        ...
        models = []
    presets = [
        {"key": "active", "model": OLLAMA_MODEL, "purpose": "Environment-selected local Ollama model for offline tutoring"},
        {"key": "fallback", "model": DANILO_AI_FALLBACK_MODEL, "purpose": "Optional backup model attempted if the active model fails"},
        {"key": "gguf", "model": DANILO_AI_PRIMARY_MODEL, "purpose": "Expected local GGUF asset when using a custom Ollama model"},
    ]
    return {
        "runtime": DANILO_AI_RUNTIME,
        ...
        "presets": presets,
        "queueSlots": _AI_MAX_CONCURRENT,
        "maxLoadedModels": _env_int("OLLAMA_MAX_LOADED_MODELS", 1, minimum=1, maximum=4),
        "contextTokens": OLLAMA_NUM_CTX,
        "inferenceThreads": OLLAMA_NUM_THREADS,
        "gpuLayers": OLLAMA_NUM_GPU,
        "batchSize": OLLAMA_NUM_BATCH,
        ...
        "hardwareProfile": _HARDWARE["profile"],
        "hardware": _HARDWARE,
        "keepAlive": os.getenv("OLLAMA_KEEP_ALIVE", "10m"),
    }
