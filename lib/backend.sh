...
def ollama_chat_payload(prompt: str, mode: str, *, stream: bool, memory: list[dict] | None = None, model: str | None = None) -> dict:
...
def admin_ai_models(current_user: User = Depends(get_current_user)) -> dict:
    ...
    return {
        ...
        "presets": presets,
        "queueSlots": _AI_MAX_CONCURRENT,
        "maxLoadedModels": int(os.getenv("OLLAMA_MAX_LOADED_MODELS", "1")),
