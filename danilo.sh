...
run_full_install() {
  ...
  run_logged_function "install_base_dependencies" install_base_dependencies
  ...
  run_logged_function "verify_mode" verify_mode || true
...
main() {
  ...
  trap 'handle_fatal_error $? ${LINENO:-0} "Uncaught Script Exit"' EXIT
