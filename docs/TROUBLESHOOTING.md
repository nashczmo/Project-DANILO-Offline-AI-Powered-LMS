...
- AI runtime offline: check `sudo grep '^DANILO_AI_RUNTIME=' /opt/danilo/app/.env`, then inspect Ollama logs with `sudo docker compose -f /opt/danilo/app/docker-compose.yml -p danilo logs --tail=120 ollama`.
