...
export default function TeacherGrades() {
  ...
  const [newAssignment, setNewAssignment] = useState({ term: "Term 1", gradingPeriod: "Midterm", component: "", maxScore: 100, weight: 1 });
  ...
  const uniqueAssignments = useMemo(() => {
    ...
    gradebook.entries.forEach(e => {
      const key = `${e.term}-${e.gradingPeriod}-${e.component}`;
      if (!map.has(key)) {
        map.set(key, { term: e.term, gradingPeriod: e.gradingPeriod || "Midterm", component: e.component, maxScore: e.maxScore, weight: e.weight });
      }
    });
  ...
  const handleSelectComponent = (val) => {
    if (val === "NEW") {
      ...
      const current = uniqueAssignments.find(a => `${a.term}-${a.gradingPeriod}-${a.component}` === val);
      if (current && gradebook) {
        const gradesMap = {};
        gradebook.students.forEach(s => {
          const entry = gradebook.entries.find(e => e.studentId === s.id && e.term === current.term && e.gradingPeriod === current.gradingPeriod && e.component === current.component);
          gradesMap[s.id] = {
  ...
  const handleSaveGrades = async (e) => {
    ...
    let targetAssignment = isCreating ? newAssignment : uniqueAssignments.find(a => `${a.term}-${a.gradingPeriod}-${a.component}` === selectedComponent);
    ...
    try {
      const promises = gradebook.students.map(async (student) => {
        ...
        const payload = {
          ...
          gradingPeriod: targetAssignment.gradingPeriod,
      ...
      setIsCreating(false);
      setSelectedComponent(`${targetAssignment.term}-${targetAssignment.gradingPeriod}-${targetAssignment.component}`);
      ...
      newGradebook.students.forEach(s => {
        const entry = newGradebook.entries.find(e => e.studentId === s.id && e.term === targetAssignment.term && e.gradingPeriod === targetAssignment.gradingPeriod && e.component === targetAssignment.component);
        gradesMap[s.id] = {
          score: entry ? entry.score : "",
          remarks: entry ? entry.remarks : "",
          gradeId: entry ? entry.id : null,
  ...
  const currentMaxScore = isCreating ? newAssignment.maxScore : (uniqueAssignments.find(a => `${a.term}-${a.gradingPeriod}-${a.component}` === selectedComponent)?.maxScore || 100);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {selectedCourseId && !gradebookLoading && gradebook && (
        <Card className="animate-fade-in">
            ...
            <div className="flex-1">
              ...
              >
                ...
                {uniqueAssignments.map(a => (
                  <option key={`${a.term}-${a.gradingPeriod}-${a.component}`} value={`${a.term}-${a.gradingPeriod}-${a.component}`}>
                    [{a.term} - {a.gradingPeriod}] {a.component} (Max: {a.maxScore}, Wt: {a.weight})
                  </option>
                ))}
          ...
          {isCreating && (
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 mb-6 p-4 bg-[#F8F9FA] rounded-xl border border-[#E0E0E0]">
              <div>
              ...
              <div>
                <label className="block text-xs font-bold text-[#5F6368] mb-1">Period</label>
                <select className="dn-input" value={newAssignment.gradingPeriod} onChange={e => setNewAssignment({...newAssignment, gradingPeriod: e.target.value})}>
