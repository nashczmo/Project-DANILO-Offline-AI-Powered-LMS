    ...
    <section className="space-y-5" aria-label="People Management">
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit">
        {["all", "student", "teacher", "admin"].map((r) => (
          <button key={r} onClick={() => setTab(r)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${tab === r ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
      ...
      <div className="dn-card p-5">
        ...
        <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-3">
          ...
          <Field label="Username">
            <input className={`dn-input ${edit ? "bg-slate-50 text-slate-400 cursor-not-allowed" : ""}`} value={form.username} onChange={(e) => { if (!edit) setForm({ ...form, username: e.target.value }); }} readOnly={!!edit} placeholder="Auto-generated from name + section" autoCapitalize="none" />
          </Field>
              ...
              <tbody className="divide-y divide-slate-100">
                {visible.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-slate-400 text-xs hidden sm:table-cell">{u.educationLevel || ""} {u.gradeLevel || ""} {u.strand || ""}</td>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {courses.map((c) => (
              <div key={c.id} className="rounded-xl border border-slate-200 bg-slate-50/50 p-4 hover:bg-white hover:shadow-sm transition-all">
                ...
                <p className="text-xs text-slate-400 mt-2">{c.teacherName || "Unassigned"} &middot; {c.studentTotal ?? 0} learners &middot; {c.moduleTotal ?? 0} modules</p>
  ...
  return (
    <section className="space-y-5" aria-label="Section Management">
      ...
      <div className="dn-card p-5">
        ...
        {loading ? <div className="text-center py-6 text-sm text-slate-400">Loading...</div> : sections.length ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {sections.map((s) => (
  ...
  return (
    <section className="space-y-5" aria-label="Enrollment Management">
      ...
      <div className="dn-card p-5">
        ...
        {loading ? <div className="text-center py-6 text-sm text-slate-400">Loading...</div> : rows.length > 0 ? (
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <table className="w-full text-left text-sm">
              <thead><tr className="bg-slate-50"><th className="px-4 py-2.5 dn-overline">Learner</th><th className="px-4 py-2.5 dn-overline">Subject</th><th className="px-4 py-2.5 dn-overline">Section</th><th className="px-4 py-2.5 dn-overline">Actions</th></tr></thead>
              <tbody className="divide-y divide-slate-100">
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-2.5 font-medium text-slate-800">{r.studentName || r.fullName}</td>
                    <td className="px-4 py-2.5 text-slate-600">{r.courseTitle || r.courseCode || r.courseId}</td>
                    <td className="px-4 py-2.5 text-slate-500 text-xs">{r.sectionName || "—"}</td>
    ...
    <section aria-label="Assessments">
      ...
      {assignments.length === 0 ? (
        ...
        <div className="dn-card p-5">
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <table className="w-full text-left text-sm">
              <thead><tr className="bg-slate-50"><th className="px-4 py-2.5 dn-overline">Title</th><th className="px-4 py-2.5 dn-overline">Subject</th><th className="px-4 py-2.5 dn-overline">Type</th><th className="px-4 py-2.5 dn-overline">Points</th><th className="px-4 py-2.5 dn-overline">Due</th></tr></thead>
              <tbody className="divide-y divide-slate-100">
                {assignments.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-2.5 font-medium text-slate-800">{a.title}</td>
                    <td className="px-4 py-2.5 text-slate-600">{a.courseTitle || a.subject || "—"}</td>
                    <td className="px-4 py-2.5"><Badge>{a.type || "Assignment"}</Badge></td>
                    <td className="px-4 py-2.5 text-slate-500">{a.points ?? "—"}</td>
                    <td className="px-4 py-2.5 text-slate-400 text-xs">{a.dueDate ? new Date(a.dueDate).toLocaleDateString() : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="dn-card p-5">
        ...
        {departments.length > 0 ? (
          <div className="grid gap-2">
            {departments.map((d) => (
              <div key={d.id} className="flex items-center justify-between rounded-lg border border-slate-200 px-4 py-3 bg-white">
    ...
    <section aria-label="Reports">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <button onClick={() => downloadReport("/admin/reports/users", "learners-faculty.csv", token)} className="dn-card p-4 text-left hover:shadow-md transition">
          ...
          <h3 className="text-sm font-semibold text-slate-900">Learners & Faculty</h3>
          <p className="text-xs text-slate-500 mt-0.5">CSV export of all accounts</p>
        </button>
        <button onClick={() => downloadReport("/admin/reports/grades", "grades.csv", token)} className="dn-card p-4 text-left hover:shadow-md transition">
          <div className="w-10 h-10 rounded-xl bg-success-50 flex items-center justify-center mb-3 text-success-600">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-900">Grades</h3>
          <p className="text-xs text-slate-500 mt-0.5">CSV export of all grade records</p>
        </button>
        <button onClick={() => downloadReport("/admin/reports/courses", "subjects.csv", token)} className="dn-card p-4 text-left hover:shadow-md transition">
          <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center mb-3 text-slate-600">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" /></svg>
          </div>
          <h3 className="text-sm font-semibold text-slate-900">Subjects</h3>
          <p className="text-xs text-slate-500 mt-0.5">CSV export of subject catalog</p>
        </button>
      </div>
    </section>
  );
  ...
  if (loading && !status) {
    return (
      <section aria-label="System Status">
        ...
        <div className="dn-card p-8 text-center text-sm text-slate-400">Loading system status...</div>
      </section>
    );
  }
      ...
      {status?.aiStatus && (
        <div className="dn-card p-4">
          <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full ${status.aiStatus.ready ? "bg-success-500" : "bg-danger-500"}`} />
            <div>
              <p className="text-sm font-medium text-slate-800">AI Model: {status.aiStatus.model || "DANILO"}</p>
              <p className="text-xs text-slate-500 mt-0.5">{status.aiStatus.ready ? "Ready for offline inference" : status.aiStatus.message || "Not available"}</p>
            </div>
          </div>
        </div>
      )}
        ...
        {logs.length > 0 ? (
          <div className="overflow-x-auto rounded-lg border border-slate-200 max-h-[400px] overflow-y-auto">
            <table className="w-full text-left text-xs">
              <thead className="sticky top-0 bg-slate-50"><tr><th className="px-4 py-2 dn-overline">Time</th><th className="px-4 py-2 dn-overline">Level</th><th className="px-4 py-2 dn-overline">Message</th></tr></thead>
              <tbody className="divide-y divide-slate-100">
                {logs.map((l, i) => (
                  <tr key={i} className="hover:bg-slate-50/50">
                    <td className="px-4 py-2 text-slate-500 whitespace-nowrap">{l.time ? new Date(l.time).toLocaleString() : "—"}</td>
                    <td className="px-4 py-2"><Badge tone={l.level === "ERROR" ? "red" : l.level === "WARN" ? "yellow" : "green"}>{l.level}</Badge></td>
                    <td className="px-4 py-2 text-slate-700">{l.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="space-y-3">
        ...
        {items.map((item) => (
          <div key={item.id} className="dn-card p-4">
            ...
            <p className="text-xs text-slate-400 mt-2">{item.createdAt ? new Date(item.createdAt).toLocaleString() : ""}</p>
          </div>
        ))}
      </div>
    </section>
    ...
    <section className="space-y-5" aria-label="Announcements">
      ...
      <div className="space-y-3">
        ...
        {items.map((item) => (
          <div key={item.id} className="dn-card p-4">
            <p className="text-sm font-semibold text-slate-900">{item.title}</p>
            <p className="text-xs text-slate-500 mb-1">{item.courseTitle || item.courseCode || ""}</p>
            <p className="text-sm text-slate-600 mt-1 whitespace-pre-wrap">{item.body}</p>
            <p className="text-xs text-slate-400 mt-2">{item.createdAt ? new Date(item.createdAt).toLocaleString() : ""}</p>
          </div>
        ))}
