...
export function Stat({ label, value, accent }) {
  ...
  return (
    <div className="dn-card p-4 text-center">
      <p className={`text-2xl font-bold tracking-tight ${accent ? "text-primary-600" : "text-slate-900"}`}>{displayValue}</p>
...
export function Empty({ icon, title, body }) {
  return (
    <div className="dn-card p-10 text-center">
      <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center mx-auto mb-4 shadow-inner">
...
export function Badge({ children, tone = "default" }) {
  ...
  return <span className={`dn-badge ${tones[tone] || tones.default}`}>{children}</span>;
}
...
export function SummaryCard({ label, value, icon, tone = "primary" }) {
  ...
  return (
    <div className="dn-card p-4 flex items-center gap-3">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${toneMap[tone] || toneMap.primary}`}>{icon}</div>
      <div>
...
export function InlineError({ message }) {
  ...
  return <p className="text-sm text-danger-600 mt-1">{message}</p>;
}
