    ...
    for file in files:
        if file.endswith('.jsx') or file.endswith('.js'):
            with open(os.path.join(root, file), 'r', encoding='utf-8') as f:
                content = f.read()
                # Find both apiRequest and fetch
                matches = re.findall(r'(?:apiRequest|fetch)\(\s*[\'\"\`](/?[^\'\"\`\?]+)', content)
                for m in matches:
                    # Clean up ${...} parts
                    clean_path = re.sub(r'\$\{.*?\}', '{id}', m)
                    frontend_endpoints.add(clean_path)
...
print('Frontend called endpoints (with {id} for vars):')
